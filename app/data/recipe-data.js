import Recipe1 from '~/assets/images/Recipe1.png'
import Recipe2 from '~/assets/images/Recipe2.png'

export const recipedata = [
  {
    url: 'recipe1',
    svg: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="28"
        height="28"
        viewBox="0 0 28 28"
        fill="none"
      >
        <g clipPath="url(#clip0_5095_3714)">
          <path
            d="M14 2.33331C20.4435 2.33331 25.6667 7.55648 25.6667 14C25.6667 20.4435 20.4435 25.6666 14 25.6666C7.55651 25.6666 2.33334 20.4435 2.33334 14C2.33334 7.55648 7.55651 2.33331 14 2.33331ZM14 4.66665C11.5247 4.66665 9.15069 5.64998 7.40035 7.40032C5.65001 9.15066 4.66668 11.5246 4.66668 14C4.66668 16.4753 5.65001 18.8493 7.40035 20.5996C9.15069 22.35 11.5247 23.3333 14 23.3333C16.4754 23.3333 18.8493 22.35 20.5997 20.5996C22.35 18.8493 23.3333 16.4753 23.3333 14C23.3333 11.5246 22.35 9.15066 20.5997 7.40032C18.8493 5.64998 16.4754 4.66665 14 4.66665ZM14 6.99998C14.2858 7.00002 14.5616 7.10493 14.7751 7.29481C14.9887 7.4847 15.1251 7.74635 15.1585 8.03015L15.1667 8.16665V13.517L18.3248 16.6751C18.5341 16.8851 18.6556 17.1668 18.6646 17.4631C18.6737 17.7594 18.5696 18.048 18.3736 18.2703C18.1775 18.4926 17.9042 18.632 17.6091 18.6601C17.314 18.6882 17.0193 18.603 16.7848 18.4216L16.6752 18.3248L13.1752 14.8248C12.9939 14.6433 12.8774 14.4071 12.8438 14.1528L12.8333 14V8.16665C12.8333 7.85723 12.9563 7.56048 13.1751 7.34169C13.3938 7.1229 13.6906 6.99998 14 6.99998Z"
            fill="#BF4745"
          />
        </g>
        <defs>
          <clipPath id="clip0_5095_3714">
            <rect width="28" height="28" fill="white" />
          </clipPath>
        </defs>
      </svg>
    ),
    imgs: [Recipe1, Recipe2, Recipe1, Recipe2, Recipe1, Recipe2],
    header: 'LOREM IPSUM DOLOR SIT AMET 1',
    text: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nullam finibus sapien non lobortis tincidunt.',
    recipepreptime_text: 'Prep time',
    recipepreptime_value: '5 min',
    recipetotaltime_text: 'Total time',
    recipetotaltime_value: '45 min',
    recipeserving_text: 'Servings',
    recipeserving_value: '6',
    paragraph:
      'Ut maximus felis vitae lacus bibendum, quis viverra metus lobortis. Praesent vitae scelerisque arcu. Vestibulum vestibulum velit nibh, eu egestas magna vulputate id. Nullam semper nunc non diam fringilla, quis posuere risus lacinia. Donec turpis arcu, dignissim at ex id, ultrices tempor metus. In hac habitasse platea dictumst. Curabitur malesuada lacus at risus rutrum vulputate. Fusce volutpat sit amet massa non finibus. Pellentesque ac faucibus justo. Interdum et malesuada fames ac ante ipsum primis in faucibus.',
    ingredients1: '1',
    ingredients1_value: 'Steak roll',
    ingredients2: '1/4',
    ingredients2_value:
      'Tray of<span className="text-[#6B1626] font-bold "> Just Meats Smoked Texas Brisket and marinade</span>',
    ingredients3: '1/2',
    ingredients3_value: 'Slices of pepper jack cheese',
    ingredients4: '1/2',
    ingredients4_value: 'Yellow onion',
    ingredients5: '1',
    ingredients5_value: 'Cup try colored bell peppers',
    direction_step1:
      'Combine <span className="text-[#6B1626] font-bold ">brisket</span>, onion, bell peppers and marinade in a pan over high heat. Cook until the bell peppers are bright and the brisket is tender (about 3 minutes).',
    direction_step2:
      'On a cutting board, add the brisket, onion, bell peppers, marinade and pepper jack cheese. Chop until everything is finely diced.',
    direction_step3: 'Fill a toasted steak roll with the mixture and enjoy.',
    Updated: 'May 30, 2024',
    type: 'beef',
  },
  {
    url: 'recipe2',
    svg: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="28"
        height="28"
        viewBox="0 0 28 28"
        fill="none"
      >
        <g clipPath="url(#clip0_5095_3714)">
          <path
            d="M14 2.33331C20.4435 2.33331 25.6667 7.55648 25.6667 14C25.6667 20.4435 20.4435 25.6666 14 25.6666C7.55651 25.6666 2.33334 20.4435 2.33334 14C2.33334 7.55648 7.55651 2.33331 14 2.33331ZM14 4.66665C11.5247 4.66665 9.15069 5.64998 7.40035 7.40032C5.65001 9.15066 4.66668 11.5246 4.66668 14C4.66668 16.4753 5.65001 18.8493 7.40035 20.5996C9.15069 22.35 11.5247 23.3333 14 23.3333C16.4754 23.3333 18.8493 22.35 20.5997 20.5996C22.35 18.8493 23.3333 16.4753 23.3333 14C23.3333 11.5246 22.35 9.15066 20.5997 7.40032C18.8493 5.64998 16.4754 4.66665 14 4.66665ZM14 6.99998C14.2858 7.00002 14.5616 7.10493 14.7751 7.29481C14.9887 7.4847 15.1251 7.74635 15.1585 8.03015L15.1667 8.16665V13.517L18.3248 16.6751C18.5341 16.8851 18.6556 17.1668 18.6646 17.4631C18.6737 17.7594 18.5696 18.048 18.3736 18.2703C18.1775 18.4926 17.9042 18.632 17.6091 18.6601C17.314 18.6882 17.0193 18.603 16.7848 18.4216L16.6752 18.3248L13.1752 14.8248C12.9939 14.6433 12.8774 14.4071 12.8438 14.1528L12.8333 14V8.16665C12.8333 7.85723 12.9563 7.56048 13.1751 7.34169C13.3938 7.1229 13.6906 6.99998 14 6.99998Z"
            fill="#BF4745"
          />
        </g>
        <defs>
          <clipPath id="clip0_5095_3714">
            <rect width="28" height="28" fill="white" />
          </clipPath>
        </defs>
      </svg>
    ),
    imgs: [Recipe1, Recipe2, Recipe1, Recipe2, Recipe1, Recipe2],
    header: 'LOREM IPSUM DOLOR SIT AMET 2',
    text: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nullam finibus sapien non lobortis tincidunt.',
    recipepreptime_text: 'Prep time',
    recipepreptime_value: '5 min',
    recipetotaltime_text: 'Total time',
    recipetotaltime_value: '45 min',
    recipeserving_text: 'Servings',
    recipeserving_value: '6',
    paragraph:
      'Ut maximus felis vitae lacus bibendum, quis viverra metus lobortis. Praesent vitae scelerisque arcu. Vestibulum vestibulum velit nibh, eu egestas magna vulputate id. Nullam semper nunc non diam fringilla, quis posuere risus lacinia. Donec turpis arcu, dignissim at ex id, ultrices tempor metus. In hac habitasse platea dictumst. Curabitur malesuada lacus at risus rutrum vulputate. Fusce volutpat sit amet massa non finibus. Pellentesque ac faucibus justo. Interdum et malesuada fames ac ante ipsum primis in faucibus.',
    ingredients1: '1',
    ingredients1_value: 'Steak roll',
    ingredients2: '1/4',
    ingredients2_value:
      'Tray of<span className="text-[#6B1626] font-bold "> Just Meats Smoked Texas Brisket and marinade</span>',
    ingredients3: '1/2',
    ingredients3_value: 'Slices of pepper jack cheese',
    ingredients4: '1/2',
    ingredients4_value: 'Yellow onion',
    ingredients5: '1',
    ingredients5_value: 'Cup try colored bell peppers',
    direction_step1:
      'Combine <span className="text-[#6B1626] font-bold ">brisket</span>, onion, bell peppers and marinade in a pan over high heat. Cook until the bell peppers are bright and the brisket is tender (about 3 minutes).',
    direction_step2:
      'On a cutting board, add the brisket, onion, bell peppers, marinade and pepper jack cheese. Chop until everything is finely diced.',
    direction_step3: 'Fill a toasted steak roll with the mixture and enjoy.',
    Updated: 'May 30, 2024',
    type: 'beef',
  },
  {
    url: 'recipe3',
    svg: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="28"
        height="28"
        viewBox="0 0 28 28"
        fill="none"
      >
        <g clipPath="url(#clip0_5095_3714)">
          <path
            d="M14 2.33331C20.4435 2.33331 25.6667 7.55648 25.6667 14C25.6667 20.4435 20.4435 25.6666 14 25.6666C7.55651 25.6666 2.33334 20.4435 2.33334 14C2.33334 7.55648 7.55651 2.33331 14 2.33331ZM14 4.66665C11.5247 4.66665 9.15069 5.64998 7.40035 7.40032C5.65001 9.15066 4.66668 11.5246 4.66668 14C4.66668 16.4753 5.65001 18.8493 7.40035 20.5996C9.15069 22.35 11.5247 23.3333 14 23.3333C16.4754 23.3333 18.8493 22.35 20.5997 20.5996C22.35 18.8493 23.3333 16.4753 23.3333 14C23.3333 11.5246 22.35 9.15066 20.5997 7.40032C18.8493 5.64998 16.4754 4.66665 14 4.66665ZM14 6.99998C14.2858 7.00002 14.5616 7.10493 14.7751 7.29481C14.9887 7.4847 15.1251 7.74635 15.1585 8.03015L15.1667 8.16665V13.517L18.3248 16.6751C18.5341 16.8851 18.6556 17.1668 18.6646 17.4631C18.6737 17.7594 18.5696 18.048 18.3736 18.2703C18.1775 18.4926 17.9042 18.632 17.6091 18.6601C17.314 18.6882 17.0193 18.603 16.7848 18.4216L16.6752 18.3248L13.1752 14.8248C12.9939 14.6433 12.8774 14.4071 12.8438 14.1528L12.8333 14V8.16665C12.8333 7.85723 12.9563 7.56048 13.1751 7.34169C13.3938 7.1229 13.6906 6.99998 14 6.99998Z"
            fill="#BF4745"
          />
        </g>
        <defs>
          <clipPath id="clip0_5095_3714">
            <rect width="28" height="28" fill="white" />
          </clipPath>
        </defs>
      </svg>
    ),
    imgs: [Recipe1, Recipe2, Recipe1, Recipe2, Recipe1, Recipe2],
    header: 'LOREM IPSUM DOLOR SIT AMET 3',
    text: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nullam finibus sapien non lobortis tincidunt.',
    recipepreptime_text: 'Prep time',
    recipepreptime_value: '5 min',
    recipetotaltime_text: 'Total time',
    recipetotaltime_value: '45 min',
    recipeserving_text: 'Servings',
    recipeserving_value: '6',
    paragraph:
      'Ut maximus felis vitae lacus bibendum, quis viverra metus lobortis. Praesent vitae scelerisque arcu. Vestibulum vestibulum velit nibh, eu egestas magna vulputate id. Nullam semper nunc non diam fringilla, quis posuere risus lacinia. Donec turpis arcu, dignissim at ex id, ultrices tempor metus. In hac habitasse platea dictumst. Curabitur malesuada lacus at risus rutrum vulputate. Fusce volutpat sit amet massa non finibus. Pellentesque ac faucibus justo. Interdum et malesuada fames ac ante ipsum primis in faucibus.',
    ingredients1: '1',
    ingredients1_value: 'Steak roll',
    ingredients2: '1/4',
    ingredients2_value:
      'Tray of<span className="text-[#6B1626] font-bold "> Just Meats Smoked Texas Brisket and marinade</span>',
    ingredients3: '1/2',
    ingredients3_value: 'Slices of pepper jack cheese',
    ingredients4: '1/2',
    ingredients4_value: 'Yellow onion',
    ingredients5: '1',
    ingredients5_value: 'Cup try colored bell peppers',
    direction_step1:
      'Combine <span className="text-[#6B1626] font-bold ">brisket</span>, onion, bell peppers and marinade in a pan over high heat. Cook until the bell peppers are bright and the brisket is tender (about 3 minutes).',
    direction_step2:
      'On a cutting board, add the brisket, onion, bell peppers, marinade and pepper jack cheese. Chop until everything is finely diced.',
    direction_step3: 'Fill a toasted steak roll with the mixture and enjoy.',
    Updated: 'May 05, 2024',
    type: 'chicken',
  },
  {
    url: 'recipe4',
    svg: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="28"
        height="28"
        viewBox="0 0 28 28"
        fill="none"
      >
        <g clipPath="url(#clip0_5095_3714)">
          <path
            d="M14 2.33331C20.4435 2.33331 25.6667 7.55648 25.6667 14C25.6667 20.4435 20.4435 25.6666 14 25.6666C7.55651 25.6666 2.33334 20.4435 2.33334 14C2.33334 7.55648 7.55651 2.33331 14 2.33331ZM14 4.66665C11.5247 4.66665 9.15069 5.64998 7.40035 7.40032C5.65001 9.15066 4.66668 11.5246 4.66668 14C4.66668 16.4753 5.65001 18.8493 7.40035 20.5996C9.15069 22.35 11.5247 23.3333 14 23.3333C16.4754 23.3333 18.8493 22.35 20.5997 20.5996C22.35 18.8493 23.3333 16.4753 23.3333 14C23.3333 11.5246 22.35 9.15066 20.5997 7.40032C18.8493 5.64998 16.4754 4.66665 14 4.66665ZM14 6.99998C14.2858 7.00002 14.5616 7.10493 14.7751 7.29481C14.9887 7.4847 15.1251 7.74635 15.1585 8.03015L15.1667 8.16665V13.517L18.3248 16.6751C18.5341 16.8851 18.6556 17.1668 18.6646 17.4631C18.6737 17.7594 18.5696 18.048 18.3736 18.2703C18.1775 18.4926 17.9042 18.632 17.6091 18.6601C17.314 18.6882 17.0193 18.603 16.7848 18.4216L16.6752 18.3248L13.1752 14.8248C12.9939 14.6433 12.8774 14.4071 12.8438 14.1528L12.8333 14V8.16665C12.8333 7.85723 12.9563 7.56048 13.1751 7.34169C13.3938 7.1229 13.6906 6.99998 14 6.99998Z"
            fill="#BF4745"
          />
        </g>
        <defs>
          <clipPath id="clip0_5095_3714">
            <rect width="28" height="28" fill="white" />
          </clipPath>
        </defs>
      </svg>
    ),
    imgs: [Recipe1, Recipe2, Recipe1, Recipe2, Recipe1, Recipe2],
    header: 'LOREM IPSUM DOLOR SIT AMET 4',
    text: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nullam finibus sapien non lobortis tincidunt.',
    recipepreptime_text: 'Prep time',
    recipepreptime_value: '5 min',
    recipetotaltime_text: 'Total time',
    recipetotaltime_value: '45 min',
    recipeserving_text: 'Servings',
    recipeserving_value: '6',
    paragraph:
      'Ut maximus felis vitae lacus bibendum, quis viverra metus lobortis. Praesent vitae scelerisque arcu. Vestibulum vestibulum velit nibh, eu egestas magna vulputate id. Nullam semper nunc non diam fringilla, quis posuere risus lacinia. Donec turpis arcu, dignissim at ex id, ultrices tempor metus. In hac habitasse platea dictumst. Curabitur malesuada lacus at risus rutrum vulputate. Fusce volutpat sit amet massa non finibus. Pellentesque ac faucibus justo. Interdum et malesuada fames ac ante ipsum primis in faucibus.',
    ingredients1: '1',
    ingredients1_value: 'Steak roll',
    ingredients2: '1/4',
    ingredients2_value:
      'Tray of<span className="text-[#6B1626] font-bold "> Just Meats Smoked Texas Brisket and marinade</span>',
    ingredients3: '1/2',
    ingredients3_value: 'Slices of pepper jack cheese',
    ingredients4: '1/2',
    ingredients4_value: 'Yellow onion',
    ingredients5: '1',
    ingredients5_value: 'Cup try colored bell peppers',
    direction_step1:
      'Combine <span className="text-[#6B1626] font-bold ">brisket</span>, onion, bell peppers and marinade in a pan over high heat. Cook until the bell peppers are bright and the brisket is tender (about 3 minutes).',
    direction_step2:
      'On a cutting board, add the brisket, onion, bell peppers, marinade and pepper jack cheese. Chop until everything is finely diced.',
    direction_step3: 'Fill a toasted steak roll with the mixture and enjoy.',
    Updated: 'May 03, 2024',
    type: 'pork',
  },
  {
    url: 'recipe5',
    svg: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="28"
        height="28"
        viewBox="0 0 28 28"
        fill="none"
      >
        <g clipPath="url(#clip0_5095_3714)">
          <path
            d="M14 2.33331C20.4435 2.33331 25.6667 7.55648 25.6667 14C25.6667 20.4435 20.4435 25.6666 14 25.6666C7.55651 25.6666 2.33334 20.4435 2.33334 14C2.33334 7.55648 7.55651 2.33331 14 2.33331ZM14 4.66665C11.5247 4.66665 9.15069 5.64998 7.40035 7.40032C5.65001 9.15066 4.66668 11.5246 4.66668 14C4.66668 16.4753 5.65001 18.8493 7.40035 20.5996C9.15069 22.35 11.5247 23.3333 14 23.3333C16.4754 23.3333 18.8493 22.35 20.5997 20.5996C22.35 18.8493 23.3333 16.4753 23.3333 14C23.3333 11.5246 22.35 9.15066 20.5997 7.40032C18.8493 5.64998 16.4754 4.66665 14 4.66665ZM14 6.99998C14.2858 7.00002 14.5616 7.10493 14.7751 7.29481C14.9887 7.4847 15.1251 7.74635 15.1585 8.03015L15.1667 8.16665V13.517L18.3248 16.6751C18.5341 16.8851 18.6556 17.1668 18.6646 17.4631C18.6737 17.7594 18.5696 18.048 18.3736 18.2703C18.1775 18.4926 17.9042 18.632 17.6091 18.6601C17.314 18.6882 17.0193 18.603 16.7848 18.4216L16.6752 18.3248L13.1752 14.8248C12.9939 14.6433 12.8774 14.4071 12.8438 14.1528L12.8333 14V8.16665C12.8333 7.85723 12.9563 7.56048 13.1751 7.34169C13.3938 7.1229 13.6906 6.99998 14 6.99998Z"
            fill="#BF4745"
          />
        </g>
        <defs>
          <clipPath id="clip0_5095_3714">
            <rect width="28" height="28" fill="white" />
          </clipPath>
        </defs>
      </svg>
    ),
    imgs: [Recipe1, Recipe2, Recipe1, Recipe2, Recipe1, Recipe2],
    header: 'LOREM IPSUM DOLOR SIT AMET 5',
    text: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nullam finibus sapien non lobortis tincidunt.',
    recipepreptime_text: 'Prep time',
    recipepreptime_value: '5 min',
    recipetotaltime_text: 'Total time',
    recipetotaltime_value: '45 min',
    recipeserving_text: 'Servings',
    recipeserving_value: '6',
    paragraph:
      'Ut maximus felis vitae lacus bibendum, quis viverra metus lobortis. Praesent vitae scelerisque arcu. Vestibulum vestibulum velit nibh, eu egestas magna vulputate id. Nullam semper nunc non diam fringilla, quis posuere risus lacinia. Donec turpis arcu, dignissim at ex id, ultrices tempor metus. In hac habitasse platea dictumst. Curabitur malesuada lacus at risus rutrum vulputate. Fusce volutpat sit amet massa non finibus. Pellentesque ac faucibus justo. Interdum et malesuada fames ac ante ipsum primis in faucibus.',
    ingredients1: '1',
    ingredients1_value: 'Steak roll',
    ingredients2: '1/4',
    ingredients2_value:
      'Tray of<span className="text-[#6B1626] font-bold "> Just Meats Smoked Texas Brisket and marinade</span>',
    ingredients3: '1/2',
    ingredients3_value: 'Slices of pepper jack cheese',
    ingredients4: '1/2',
    ingredients4_value: 'Yellow onion',
    ingredients5: '1',
    ingredients5_value: 'Cup try colored bell peppers',
    direction_step1:
      'Combine <span className="text-[#6B1626] font-bold ">brisket</span>, onion, bell peppers and marinade in a pan over high heat. Cook until the bell peppers are bright and the brisket is tender (about 3 minutes).',
    direction_step2:
      'On a cutting board, add the brisket, onion, bell peppers, marinade and pepper jack cheese. Chop until everything is finely diced.',
    direction_step3: 'Fill a toasted steak roll with the mixture and enjoy.',
    Updated: 'May 15, 2024',
    type: 'pork',
  },
  {
    url: 'recipe6',
    svg: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="28"
        height="28"
        viewBox="0 0 28 28"
        fill="none"
      >
        <g clipPath="url(#clip0_5095_3714)">
          <path
            d="M14 2.33331C20.4435 2.33331 25.6667 7.55648 25.6667 14C25.6667 20.4435 20.4435 25.6666 14 25.6666C7.55651 25.6666 2.33334 20.4435 2.33334 14C2.33334 7.55648 7.55651 2.33331 14 2.33331ZM14 4.66665C11.5247 4.66665 9.15069 5.64998 7.40035 7.40032C5.65001 9.15066 4.66668 11.5246 4.66668 14C4.66668 16.4753 5.65001 18.8493 7.40035 20.5996C9.15069 22.35 11.5247 23.3333 14 23.3333C16.4754 23.3333 18.8493 22.35 20.5997 20.5996C22.35 18.8493 23.3333 16.4753 23.3333 14C23.3333 11.5246 22.35 9.15066 20.5997 7.40032C18.8493 5.64998 16.4754 4.66665 14 4.66665ZM14 6.99998C14.2858 7.00002 14.5616 7.10493 14.7751 7.29481C14.9887 7.4847 15.1251 7.74635 15.1585 8.03015L15.1667 8.16665V13.517L18.3248 16.6751C18.5341 16.8851 18.6556 17.1668 18.6646 17.4631C18.6737 17.7594 18.5696 18.048 18.3736 18.2703C18.1775 18.4926 17.9042 18.632 17.6091 18.6601C17.314 18.6882 17.0193 18.603 16.7848 18.4216L16.6752 18.3248L13.1752 14.8248C12.9939 14.6433 12.8774 14.4071 12.8438 14.1528L12.8333 14V8.16665C12.8333 7.85723 12.9563 7.56048 13.1751 7.34169C13.3938 7.1229 13.6906 6.99998 14 6.99998Z"
            fill="#BF4745"
          />
        </g>
        <defs>
          <clipPath id="clip0_5095_3714">
            <rect width="28" height="28" fill="white" />
          </clipPath>
        </defs>
      </svg>
    ),
    imgs: [Recipe1, Recipe2, Recipe1, Recipe2, Recipe1, Recipe2],
    header: 'LOREM IPSUM DOLOR SIT AMET 6',
    text: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nullam finibus sapien non lobortis tincidunt.',
    recipepreptime_text: 'Prep time',
    recipepreptime_value: '5 min',
    recipetotaltime_text: 'Total time',
    recipetotaltime_value: '45 min',
    recipeserving_text: 'Servings',
    recipeserving_value: '6',
    paragraph:
      'Ut maximus felis vitae lacus bibendum, quis viverra metus lobortis. Praesent vitae scelerisque arcu. Vestibulum vestibulum velit nibh, eu egestas magna vulputate id. Nullam semper nunc non diam fringilla, quis posuere risus lacinia. Donec turpis arcu, dignissim at ex id, ultrices tempor metus. In hac habitasse platea dictumst. Curabitur malesuada lacus at risus rutrum vulputate. Fusce volutpat sit amet massa non finibus. Pellentesque ac faucibus justo. Interdum et malesuada fames ac ante ipsum primis in faucibus.',
    ingredients1: '1',
    ingredients1_value: 'Steak roll',
    ingredients2: '1/4',
    ingredients2_value:
      'Tray of<span className="text-[#6B1626] font-bold "> Just Meats Smoked Texas Brisket and marinade</span>',
    ingredients3: '1/2',
    ingredients3_value: 'Slices of pepper jack cheese',
    ingredients4: '1/2',
    ingredients4_value: 'Yellow onion',
    ingredients5: '1',
    ingredients5_value: 'Cup try colored bell peppers',
    direction_step1:
      'Combine <span className="text-[#6B1626] font-bold ">brisket</span>, onion, bell peppers and marinade in a pan over high heat. Cook until the bell peppers are bright and the brisket is tender (about 3 minutes).',
    direction_step2:
      'On a cutting board, add the brisket, onion, bell peppers, marinade and pepper jack cheese. Chop until everything is finely diced.',
    direction_step3: 'Fill a toasted steak roll with the mixture and enjoy.',
    Updated: 'May 20, 2024',
    type: 'turkey',
  },
  {
    url: 'recipe7',
    svg: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="28"
        height="28"
        viewBox="0 0 28 28"
        fill="none"
      >
        <g clipPath="url(#clip0_5095_3714)">
          <path
            d="M14 2.33331C20.4435 2.33331 25.6667 7.55648 25.6667 14C25.6667 20.4435 20.4435 25.6666 14 25.6666C7.55651 25.6666 2.33334 20.4435 2.33334 14C2.33334 7.55648 7.55651 2.33331 14 2.33331ZM14 4.66665C11.5247 4.66665 9.15069 5.64998 7.40035 7.40032C5.65001 9.15066 4.66668 11.5246 4.66668 14C4.66668 16.4753 5.65001 18.8493 7.40035 20.5996C9.15069 22.35 11.5247 23.3333 14 23.3333C16.4754 23.3333 18.8493 22.35 20.5997 20.5996C22.35 18.8493 23.3333 16.4753 23.3333 14C23.3333 11.5246 22.35 9.15066 20.5997 7.40032C18.8493 5.64998 16.4754 4.66665 14 4.66665ZM14 6.99998C14.2858 7.00002 14.5616 7.10493 14.7751 7.29481C14.9887 7.4847 15.1251 7.74635 15.1585 8.03015L15.1667 8.16665V13.517L18.3248 16.6751C18.5341 16.8851 18.6556 17.1668 18.6646 17.4631C18.6737 17.7594 18.5696 18.048 18.3736 18.2703C18.1775 18.4926 17.9042 18.632 17.6091 18.6601C17.314 18.6882 17.0193 18.603 16.7848 18.4216L16.6752 18.3248L13.1752 14.8248C12.9939 14.6433 12.8774 14.4071 12.8438 14.1528L12.8333 14V8.16665C12.8333 7.85723 12.9563 7.56048 13.1751 7.34169C13.3938 7.1229 13.6906 6.99998 14 6.99998Z"
            fill="#BF4745"
          />
        </g>
        <defs>
          <clipPath id="clip0_5095_3714">
            <rect width="28" height="28" fill="white" />
          </clipPath>
        </defs>
      </svg>
    ),
    imgs: [Recipe1, Recipe2, Recipe1, Recipe2, Recipe1, Recipe2],
    header: 'LOREM IPSUM DOLOR SIT AMET 6',
    text: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nullam finibus sapien non lobortis tincidunt.',
    recipepreptime_text: 'Prep time',
    recipepreptime_value: '5 min',
    recipetotaltime_text: 'Total time',
    recipetotaltime_value: '45 min',
    recipeserving_text: 'Servings',
    recipeserving_value: '6',
    paragraph:
      'Ut maximus felis vitae lacus bibendum, quis viverra metus lobortis. Praesent vitae scelerisque arcu. Vestibulum vestibulum velit nibh, eu egestas magna vulputate id. Nullam semper nunc non diam fringilla, quis posuere risus lacinia. Donec turpis arcu, dignissim at ex id, ultrices tempor metus. In hac habitasse platea dictumst. Curabitur malesuada lacus at risus rutrum vulputate. Fusce volutpat sit amet massa non finibus. Pellentesque ac faucibus justo. Interdum et malesuada fames ac ante ipsum primis in faucibus.',
    ingredients1: '1',
    ingredients1_value: 'Steak roll',
    ingredients2: '1/4',
    ingredients2_value:
      'Tray of<span className="text-[#6B1626] font-bold "> Just Meats Smoked Texas Brisket and marinade</span>',
    ingredients3: '1/2',
    ingredients3_value: 'Slices of pepper jack cheese',
    ingredients4: '1/2',
    ingredients4_value: 'Yellow onion',
    ingredients5: '1',
    ingredients5_value: 'Cup try colored bell peppers',
    direction_step1:
      'Combine <span className="text-[#6B1626] font-bold ">brisket</span>, onion, bell peppers and marinade in a pan over high heat. Cook until the bell peppers are bright and the brisket is tender (about 3 minutes).',
    direction_step2:
      'On a cutting board, add the brisket, onion, bell peppers, marinade and pepper jack cheese. Chop until everything is finely diced.',
    direction_step3: 'Fill a toasted steak roll with the mixture and enjoy.',
    Updated: 'May 30, 2024',
    type: 'turkey',
  },
  {
    url: 'recipe8',
    svg: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="28"
        height="28"
        viewBox="0 0 28 28"
        fill="none"
      >
        <g clipPath="url(#clip0_5095_3714)">
          <path
            d="M14 2.33331C20.4435 2.33331 25.6667 7.55648 25.6667 14C25.6667 20.4435 20.4435 25.6666 14 25.6666C7.55651 25.6666 2.33334 20.4435 2.33334 14C2.33334 7.55648 7.55651 2.33331 14 2.33331ZM14 4.66665C11.5247 4.66665 9.15069 5.64998 7.40035 7.40032C5.65001 9.15066 4.66668 11.5246 4.66668 14C4.66668 16.4753 5.65001 18.8493 7.40035 20.5996C9.15069 22.35 11.5247 23.3333 14 23.3333C16.4754 23.3333 18.8493 22.35 20.5997 20.5996C22.35 18.8493 23.3333 16.4753 23.3333 14C23.3333 11.5246 22.35 9.15066 20.5997 7.40032C18.8493 5.64998 16.4754 4.66665 14 4.66665ZM14 6.99998C14.2858 7.00002 14.5616 7.10493 14.7751 7.29481C14.9887 7.4847 15.1251 7.74635 15.1585 8.03015L15.1667 8.16665V13.517L18.3248 16.6751C18.5341 16.8851 18.6556 17.1668 18.6646 17.4631C18.6737 17.7594 18.5696 18.048 18.3736 18.2703C18.1775 18.4926 17.9042 18.632 17.6091 18.6601C17.314 18.6882 17.0193 18.603 16.7848 18.4216L16.6752 18.3248L13.1752 14.8248C12.9939 14.6433 12.8774 14.4071 12.8438 14.1528L12.8333 14V8.16665C12.8333 7.85723 12.9563 7.56048 13.1751 7.34169C13.3938 7.1229 13.6906 6.99998 14 6.99998Z"
            fill="#BF4745"
          />
        </g>
        <defs>
          <clipPath id="clip0_5095_3714">
            <rect width="28" height="28" fill="white" />
          </clipPath>
        </defs>
      </svg>
    ),
    imgs: [Recipe1, Recipe2, Recipe1, Recipe2, Recipe1, Recipe2],
    header: 'LOREM IPSUM DOLOR SIT AMET 6',
    text: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nullam finibus sapien non lobortis tincidunt.',
    recipepreptime_text: 'Prep time',
    recipepreptime_value: '5 min',
    recipetotaltime_text: 'Total time',
    recipetotaltime_value: '45 min',
    recipeserving_text: 'Servings',
    recipeserving_value: '6',
    paragraph:
      'Ut maximus felis vitae lacus bibendum, quis viverra metus lobortis. Praesent vitae scelerisque arcu. Vestibulum vestibulum velit nibh, eu egestas magna vulputate id. Nullam semper nunc non diam fringilla, quis posuere risus lacinia. Donec turpis arcu, dignissim at ex id, ultrices tempor metus. In hac habitasse platea dictumst. Curabitur malesuada lacus at risus rutrum vulputate. Fusce volutpat sit amet massa non finibus. Pellentesque ac faucibus justo. Interdum et malesuada fames ac ante ipsum primis in faucibus.',
    ingredients1: '1',
    ingredients1_value: 'Steak roll',
    ingredients2: '1/4',
    ingredients2_value:
      'Tray of<span className="text-[#6B1626] font-bold "> Just Meats Smoked Texas Brisket and marinade</span>',
    ingredients3: '1/2',
    ingredients3_value: 'Slices of pepper jack cheese',
    ingredients4: '1/2',
    ingredients4_value: 'Yellow onion',
    ingredients5: '1',
    ingredients5_value: 'Cup try colored bell peppers',
    direction_step1:
      'Combine <span className="text-[#6B1626] font-bold ">brisket</span>, onion, bell peppers and marinade in a pan over high heat. Cook until the bell peppers are bright and the brisket is tender (about 3 minutes).',
    direction_step2:
      'On a cutting board, add the brisket, onion, bell peppers, marinade and pepper jack cheese. Chop until everything is finely diced.',
    direction_step3: 'Fill a toasted steak roll with the mixture and enjoy.',
    Updated: 'May 20, 2024',
    type: 'turkey',
  },
  {
    url: 'recipe9',
    svg: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="28"
        height="28"
        viewBox="0 0 28 28"
        fill="none"
      >
        <g clipPath="url(#clip0_5095_3714)">
          <path
            d="M14 2.33331C20.4435 2.33331 25.6667 7.55648 25.6667 14C25.6667 20.4435 20.4435 25.6666 14 25.6666C7.55651 25.6666 2.33334 20.4435 2.33334 14C2.33334 7.55648 7.55651 2.33331 14 2.33331ZM14 4.66665C11.5247 4.66665 9.15069 5.64998 7.40035 7.40032C5.65001 9.15066 4.66668 11.5246 4.66668 14C4.66668 16.4753 5.65001 18.8493 7.40035 20.5996C9.15069 22.35 11.5247 23.3333 14 23.3333C16.4754 23.3333 18.8493 22.35 20.5997 20.5996C22.35 18.8493 23.3333 16.4753 23.3333 14C23.3333 11.5246 22.35 9.15066 20.5997 7.40032C18.8493 5.64998 16.4754 4.66665 14 4.66665ZM14 6.99998C14.2858 7.00002 14.5616 7.10493 14.7751 7.29481C14.9887 7.4847 15.1251 7.74635 15.1585 8.03015L15.1667 8.16665V13.517L18.3248 16.6751C18.5341 16.8851 18.6556 17.1668 18.6646 17.4631C18.6737 17.7594 18.5696 18.048 18.3736 18.2703C18.1775 18.4926 17.9042 18.632 17.6091 18.6601C17.314 18.6882 17.0193 18.603 16.7848 18.4216L16.6752 18.3248L13.1752 14.8248C12.9939 14.6433 12.8774 14.4071 12.8438 14.1528L12.8333 14V8.16665C12.8333 7.85723 12.9563 7.56048 13.1751 7.34169C13.3938 7.1229 13.6906 6.99998 14 6.99998Z"
            fill="#BF4745"
          />
        </g>
        <defs>
          <clipPath id="clip0_5095_3714">
            <rect width="28" height="28" fill="white" />
          </clipPath>
        </defs>
      </svg>
    ),
    imgs: [Recipe1, Recipe2, Recipe1, Recipe2, Recipe1, Recipe2],
    header: 'LOREM IPSUM DOLOR SIT AMET 6',
    text: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nullam finibus sapien non lobortis tincidunt.',
    recipepreptime_text: 'Prep time',
    recipepreptime_value: '5 min',
    recipetotaltime_text: 'Total time',
    recipetotaltime_value: '45 min',
    recipeserving_text: 'Servings',
    recipeserving_value: '6',
    paragraph:
      'Ut maximus felis vitae lacus bibendum, quis viverra metus lobortis. Praesent vitae scelerisque arcu. Vestibulum vestibulum velit nibh, eu egestas magna vulputate id. Nullam semper nunc non diam fringilla, quis posuere risus lacinia. Donec turpis arcu, dignissim at ex id, ultrices tempor metus. In hac habitasse platea dictumst. Curabitur malesuada lacus at risus rutrum vulputate. Fusce volutpat sit amet massa non finibus. Pellentesque ac faucibus justo. Interdum et malesuada fames ac ante ipsum primis in faucibus.',
    ingredients1: '1',
    ingredients1_value: 'Steak roll',
    ingredients2: '1/4',
    ingredients2_value:
      'Tray of<span className="text-[#6B1626] font-bold "> Just Meats Smoked Texas Brisket and marinade</span>',
    ingredients3: '1/2',
    ingredients3_value: 'Slices of pepper jack cheese',
    ingredients4: '1/2',
    ingredients4_value: 'Yellow onion',
    ingredients5: '1',
    ingredients5_value: 'Cup try colored bell peppers',
    direction_step1:
      'Combine <span className="text-[#6B1626] font-bold ">brisket</span>, onion, bell peppers and marinade in a pan over high heat. Cook until the bell peppers are bright and the brisket is tender (about 3 minutes).',
    direction_step2:
      'On a cutting board, add the brisket, onion, bell peppers, marinade and pepper jack cheese. Chop until everything is finely diced.',
    direction_step3: 'Fill a toasted steak roll with the mixture and enjoy.',
    Updated: 'May 9, 2024',
    type: 'bison',
  },
  {
    url: 'recipe10',
    svg: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="28"
        height="28"
        viewBox="0 0 28 28"
        fill="none"
      >
        <g clipPath="url(#clip0_5095_3714)">
          <path
            d="M14 2.33331C20.4435 2.33331 25.6667 7.55648 25.6667 14C25.6667 20.4435 20.4435 25.6666 14 25.6666C7.55651 25.6666 2.33334 20.4435 2.33334 14C2.33334 7.55648 7.55651 2.33331 14 2.33331ZM14 4.66665C11.5247 4.66665 9.15069 5.64998 7.40035 7.40032C5.65001 9.15066 4.66668 11.5246 4.66668 14C4.66668 16.4753 5.65001 18.8493 7.40035 20.5996C9.15069 22.35 11.5247 23.3333 14 23.3333C16.4754 23.3333 18.8493 22.35 20.5997 20.5996C22.35 18.8493 23.3333 16.4753 23.3333 14C23.3333 11.5246 22.35 9.15066 20.5997 7.40032C18.8493 5.64998 16.4754 4.66665 14 4.66665ZM14 6.99998C14.2858 7.00002 14.5616 7.10493 14.7751 7.29481C14.9887 7.4847 15.1251 7.74635 15.1585 8.03015L15.1667 8.16665V13.517L18.3248 16.6751C18.5341 16.8851 18.6556 17.1668 18.6646 17.4631C18.6737 17.7594 18.5696 18.048 18.3736 18.2703C18.1775 18.4926 17.9042 18.632 17.6091 18.6601C17.314 18.6882 17.0193 18.603 16.7848 18.4216L16.6752 18.3248L13.1752 14.8248C12.9939 14.6433 12.8774 14.4071 12.8438 14.1528L12.8333 14V8.16665C12.8333 7.85723 12.9563 7.56048 13.1751 7.34169C13.3938 7.1229 13.6906 6.99998 14 6.99998Z"
            fill="#BF4745"
          />
        </g>
        <defs>
          <clipPath id="clip0_5095_3714">
            <rect width="28" height="28" fill="white" />
          </clipPath>
        </defs>
      </svg>
    ),
    imgs: [Recipe1, Recipe2, Recipe1, Recipe2, Recipe1, Recipe2],
    header: 'LOREM IPSUM DOLOR SIT AMET 6',
    text: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nullam finibus sapien non lobortis tincidunt.',
    recipepreptime_text: 'Prep time',
    recipepreptime_value: '5 min',
    recipetotaltime_text: 'Total time',
    recipetotaltime_value: '45 min',
    recipeserving_text: 'Servings',
    recipeserving_value: '6',
    paragraph:
      'Ut maximus felis vitae lacus bibendum, quis viverra metus lobortis. Praesent vitae scelerisque arcu. Vestibulum vestibulum velit nibh, eu egestas magna vulputate id. Nullam semper nunc non diam fringilla, quis posuere risus lacinia. Donec turpis arcu, dignissim at ex id, ultrices tempor metus. In hac habitasse platea dictumst. Curabitur malesuada lacus at risus rutrum vulputate. Fusce volutpat sit amet massa non finibus. Pellentesque ac faucibus justo. Interdum et malesuada fames ac ante ipsum primis in faucibus.',
    ingredients1: '1',
    ingredients1_value: 'Steak roll',
    ingredients2: '1/4',
    ingredients2_value:
      'Tray of<span className="text-[#6B1626] font-bold "> Just Meats Smoked Texas Brisket and marinade</span>',
    ingredients3: '1/2',
    ingredients3_value: 'Slices of pepper jack cheese',
    ingredients4: '1/2',
    ingredients4_value: 'Yellow onion',
    ingredients5: '1',
    ingredients5_value: 'Cup try colored bell peppers',
    direction_step1:
      'Combine <span className="text-[#6B1626] font-bold ">brisket</span>, onion, bell peppers and marinade in a pan over high heat. Cook until the bell peppers are bright and the brisket is tender (about 3 minutes).',
    direction_step2:
      'On a cutting board, add the brisket, onion, bell peppers, marinade and pepper jack cheese. Chop until everything is finely diced.',
    direction_step3: 'Fill a toasted steak roll with the mixture and enjoy.',
    Updated: 'May 13, 2024',
    type: 'bison',
  },
]
