export const HeatEatRepeat = () => {
  return (
    <section>        
        <div className="bg-[url('../assets/images/JustMeats_StyledProduct_Stills_Full.png')] bg-no-repeat bg-cover">
        <div className="bg-[url('../assets/images/recipe-bottom-img.png')] bg-repeat  px-[13px] py-[13px]"></div>
            <div className="sm:w-[700px] flex p-[50px] pt-[80px]">
                <div className="pr-[10px]">
                    <div className="w-[56px] h-[56px] bg-[#FF655D] rounded-[40px] font-hudson text-[#FFF] text-[36px] flex justify-center">5</div>
                </div>
                <div>
                    <div className="font-hudson sm:text-[28px] text-[24px] text-[#7D0E21]">
                        Heat. Eat. Repeat.
                    </div>
                    <div className="sm:text-[18px] text-[16px] text-[#7D0E21]">
                        We want your meat to arrive as fresh as possible, so it’s flash frozen after we cook it.
                    </div>
                    <div className="sm:text-[62px] text-[54px] text-[#F21E36]">Stays Fresh</div>
                    <div className="sm:text-[62px] text-[54px] text-[#000]">10 DAYS</div>
                    <div className="sm:text-[24px] text-[21px] text-[#7D0F21] font-bold	">Stored in Fridge</div>
                    <div className="sm:text-[62px] text-[54px] text-[#000]">90-120 DAYS</div>
                    <div className="sm:text-[24px] text-[21px] text-[#7D0F21] font-bold	">Frozen</div>
                    <div className="py-[20px]">
                    <svg xmlns="http://www.w3.org/2000/svg" width="609" height="6" viewBox="0 0 609 6" fill="none">
                        <path d="M3 3H606" stroke="black" strokeWidth="5" strokeLinecap="round" strokeDasharray="0.5 12"/>
                    </svg>
                    </div>
                    <div className="sm:text-[18px] text-[16px] text-[#000]">To thaw your meat, place it in the fridge overnight, and then reheat it using one of these three methods: </div>
                    <ul className="sm:text-[18px] text-[16px] text-[#000] py-[20px] list-decimal pl-[30px]">
                        <li>Skillet or sauté pan on medium-high heat for two minutes. </li>
                        <li>Air fryer at 350 degrees for six minutes. </li>
                        <li>Oven at 300 degrees for 30-40 minutes (make sure to stir after 20 minutes). </li>
                    </ul>
                    <div className="sm:text-[18px] text-[16px] text-[#000]">
                        Serve with some sides or a salad and voilá! 
                    </div>
                    <div className="sm:text-[18px] text-[16px] text-[#000]">
                        A delicious dinner is served.
                    </div>
                </div>
            </div>
        </div>
    </section>
  )
}
